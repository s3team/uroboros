...
type cfgi = {
  preds : (instr option, instr option list) Hashtbl.t;
...
let stub_loc : loc = { loc_label = ""; loc_addr = 0; loc_visible = true }

let read_lines (filename : string) : string list =
  File.with_file_in filename (fun input -> List.of_enum (IO.lines_of input))
...
let unify_int_list (l : int list) : int list =
  let s = List.fold_left (fun s ele -> IntSet.add ele s) IntSet.empty l in
  (* remove dups *)
...
let unify_str_list (l : string list) : string list =
  let s = List.fold_left (fun s ele -> StringSet.add ele s) StringSet.empty l in
  (* elements should be returned in increasing order *)
...
let unify_funclist_by_addr (l : func list) : func list =
  let rec help l acc =
    ...
    | h1 :: h2 :: t -> begin
        if h1.func_begin_addr == h2.func_begin_addr then begin
...
let change_op i new_op =
  match i with
  | SingleInstr (p, l, pre, tag, tags) -> SingleInstr (new_op, l, pre, tag, tags)
  | DoubleInstr (p, e, l, pre, tag, tags) ->
...
let get_cf_des i =
  match i with DoubleInstr (_, e, _, _, _, _) -> Some e | _ -> None


(** Get the first exp. e.g.: bx r1 -> r1 mov r0, r1 -> r1 add r1, r2, r3 -> r3
*)
let get_exp_1 i =
  match i with
...
(** Get the second exp. e.g.: mov r0, r1 -> r0 add r1, r2, r3 -> r2 *)
let get_exp_2 i =
  match i with
  | SingleInstr (_, _, _, _, _) -> failwith "undefined expression"
...
let dec_hex (s : int) : string = Printf.sprintf "0x%X" s
let dec_hex_sym (s : int) : string = Printf.sprintf "S_0x%X" s
...
let rec cat_from (l : string list) (f : int) (s : string) : string =
  ...
  match l with
  | h :: t -> if f == 0 then h ^ s ^ cat_from t 0 s else cat_from t (f - 1) s
  | [] -> ""
...
let get_instr_byloc (instrs : instr list) (locs : loc list) : instr list =
  ...
  let rec aux il ll acc =
    ...
    | ih :: it, lh :: lt -> begin
        ...
        if iloc.loc_addr < lh.loc_addr then aux it ll acc
        else if iloc.loc_addr = lh.loc_addr then aux it lt (ih :: acc)
        else assert false
      end
    | _, [] -> List.rev acc
    | _ -> assert false
  ...
  List.fold_left
    (fun acc l ->
      ...
      if List.nth items 0 = sec then
        let baddrs = List.nth items 1 and sizes = List.nth items 3 in
...
let f_instrs f instrs =
  let baddr = f.func_begin_addr and eaddr = f.func_end_addr in
  let rec help il acc =
    match il with
    | h :: t -> begin
        let loc = get_loc h in
        if loc.loc_addr >= baddr && loc.loc_addr < eaddr then help t (h :: acc)
        else help t acc
      end
    | [] -> List.rev acc
  in
...
let bb_instrs b instrs =
  let bloc = b.bblock_begin_loc and eloc = b.bblock_end_loc in
  let rec help il acc =
    match il with
    | h :: t -> begin
        let loc = get_loc h in
        if loc.loc_addr >= bloc.loc_addr && loc.loc_addr <= eloc.loc_addr then
          help t (h :: acc)
        else help t acc
      end
    | [] -> List.rev acc
...
let remove_over_once l =
  ...
  List.iter
    ...
    l;
  List.filter (fun ele -> if Hashtbl.find m ele > 1 then false else true) l
...
let bbn_byloc e ls =
  ...
  let rec bs min max =
    if min > max then false
      ...
      if e = mid_num then true
      else if e < mid_num then bs min (mid - 1)
      else bs (mid + 1) max
  in
...
module Algo = struct
  let sort_uniq cmp l =
    let rec aux acc l =
      match l with
      | h1 :: h2 :: t ->
          if cmp h1 h2 = 0 then aux acc (h2 :: t) else aux (h1 :: acc) (h2 :: t)
      | h :: [] -> h :: acc
      | [] -> acc
    in
...
module ELF_utils = struct
  let elf_32 () =
    ...
    String.exists l "ELF 32-bit"


  let elf_64 () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "ELF 64-bit"


  let elf_dynamic () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "dynamically linked"


  let elf_static () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "statically linked"


  let elf_unstrip () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "not stripped"


  let elf_strip () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l ", stripped"


  let elf_exe () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "LSB executable"


  let elf_lib () =
    (read_file "elf.info" |> fun l -> List.nth l 0) |> fun l ->
    String.exists l "LSB shared object"
...
module Opcode_utils = struct
  ...
  let is_control_des i =
    let aux s = String.exists s ":" in
    get_label i |> aux


  let is_func e = match e with Symbol (CallDes _) -> true | _ -> false

  let is_jmp (i : instr) : bool =
    match get_op i with
    | Intel_OP iop -> begin
        match iop with
        | Intel_ControlOP ico -> begin
            match ico with Intel_Jump JMP -> true | _ -> false
          end
        | _ -> false
      end
    | Arm_OP (ao, _, _) -> begin
        match ao with
        | Arm_ControlOP aco -> begin
            match aco with
            (* TODO: ARM: *)
            (* should be distinguished from "BX LR", which is a return. *)
            | B | BXJ -> true
            | BX when not (has_lr_reg i) -> true
            | _ -> false
          end
        | _ -> false
      end
  ...
  let is_cond_jmp op =
    ...
    | Arm_OP (ao, cond, _) -> begin
        match ao with
        | Arm_ControlOP aco -> begin
            match aco with B when cond <> None -> true | _ -> false
          end
        | _ -> false
      end
  ...
  let is_assign = function
    | Intel_OP io -> begin
        match io with Intel_CommonOP (Intel_Assign _) -> true | _ -> false
      end
  ...
  let is_stack_op = function
    | Intel_OP io -> begin
        ...
        | Intel_StackOP POPL
        | Intel_StackOP POPF
        | Intel_ControlOP LEAVE ->
            true
        | _ -> false
      end
    | Arm_OP (ao, _, _) -> begin
        match ao with Arm_StackOP _ -> true | _ -> false
      end
    | _ -> false


  let is_mov = function
    | Intel_CommonOP (Intel_Assign MOV) | Intel_CommonOP (Intel_Assign MOVL) ->
        true
    | _ -> false
  ...
  let is_call op =
    ...
    | Intel_OP iop -> begin
        ...
        | Intel_ControlOP c -> begin
            match c with CALL -> true | CALLQ -> true | _ -> false
          end
        | _ -> false
      end
    | Arm_OP (ao, _, _) -> begin
  ...
  let is_ret (i : instr) : bool =
    match get_op i with
    | Intel_OP iop -> begin
        match iop with
        | Intel_ControlOP c -> begin
            match c with RET | RETN | RETQ -> true | _ -> false
          end
        | _ -> false
      end
    | Arm_OP (aop, _, _) -> begin
        match aop with
        | Arm_ControlOP aco -> begin
            match aco with
            (* TODO: ARM: This is naive because we need to check if BX uses LR register *)
            | BX when has_lr_reg i -> true
            | _ -> false
          end
        | Arm_StackOP aso -> begin
            match aso with
            (* TODO: ARM: This is naive because we need to check if POP uses PC register *)
            | POP when has_pc_reg i -> true
            | _ -> false
          end
        | _ -> false
      end
  ...
  let is_arm_ret (inst : instr) : bool =
    match inst with
    ...
    | DoubleInstr (op, e, _, _, _, _) -> (
        ...
        | Arm_OP (Arm_StackOP POP, _, _) when has_pc_reg inst -> true
        | _ -> false)
    | _ -> false
  ...
  let is_indirect = function
    | Symbol s -> begin
        match s with StarDes _ -> true | _ -> false
  ...
  let is_control_transfer_op (i : instr) =
    let op = get_op i in
    is_call op || is_jmp i || is_cond_jmp op || is_ret i
...
module Exp_utils = struct
  let is_reg e = match e with Reg _ -> true | _ -> false
  let is_const e = match e with Const _ -> true | _ -> false
  let is_mem e = match e with Ptr _ -> true | _ -> false
end
...
module Instr_utils = struct
  ...
  let sort_il_update instrs_update =
    List.sort
      (fun (_, l1, _, _) (_, l2, _, _) -> l1.loc_addr - l2.loc_addr)
      instrs_update


  type instr_update = SUB | INSERT

  let set_update_fold i l acc = [ (i, l, INSERT, "") ] :: acc
  ...
  let eliminate_label i = update_label i ""
  ...
  let update_instrs_infront instrs instrs_update =
    let same loc1 loc2 = loc1.loc_addr = loc2.loc_addr in
    let rec help l_u l acc =
      match (l_u, l) with
      | h_u :: t_u, h :: t -> begin
          let i, loc1, ty, i_s = h_u and loc2 = get_loc h in
          if same loc1 loc2 then begin
            match ty with
            | SUB -> begin
                let open Pp_print in
                let h_s = pp_print_instr h in
                if i_s = h_s then help t_u (i :: t) acc
                else help l_u t (h :: acc)
              end
            | INSERT -> help t_u (h :: t) (i :: acc)
          end
          else help l_u t (h :: acc)
        end
      | [], l' -> List.rev_append acc l'
      | h :: t, [] -> begin failwith "error in update_instrs" end
    in
    help instrs_update instrs []


  let insert_instrument_instrs il il_update =
    il_update |> sort_il_update |> update_instrs_infront il


  let sub_single_instr i' il_update il =
    let loc = get_loc i' in
    let acc =
      List.fold_left (fun acc i -> sub_update_fold i loc i' acc) il_update []
    in
    let acc' = List.flatten acc in
    insert_instrument_instrs il acc'


  let instrument_update il_update il = insert_instrument_instrs il il_update

  let instrument_instrs_func il il_update update_func =
    il_update |> sort_il_update |> update_func il


  let insert_instr_list d il_update il =
    match d with
    | BEFORE -> instrument_instrs_func il il_update update_instrs_infront
    | AFTER -> instrument_instrs_func il il_update update_instrs_infront


  let insert_single_instr d loc il_update il =
    let acc =
      List.fold_left (fun acc i -> set_update_fold i loc acc) [] il_update
    in
    let acc' = List.flatten acc in
  ...
  let get_bb_by_last_instr i bbl =
    ...
    (*
      ...
      print_string "\n";
      *)
    List.find (fun b -> b.bblock_end_loc.loc_addr = addr) bbl


  (* generate one byte no-op padding instruction *)
  let gen_nop loc =
    SingleInstr
      ( Intel_OP (Intel_CommonOP (Intel_Other NOP)),
        loc,
        None,
        None,
        Hashtbl.create 0 )
  ...
  let gen_4_lea loc =
    TripleInstr
      ( Intel_OP (Intel_CommonOP (Intel_Assign LEA)),
        ...
        Ptr (BinOP_PLUS (Intel_Reg (Intel_CommonReg ESI), 0)),
        loc,
        None,
        None,
        Hashtbl.create 0 )
  ...
  let gen_6_lea loc =
    TripleInstr
      ( Intel_OP (Intel_CommonOP (Intel_Assign LEA)),
        ...
        Ptr (BinOP_PLUS (Intel_Reg (Intel_CommonReg EDI), 0)),
        loc,
        None,
        None,
        Hashtbl.create 0 )
  ...
  let is_mem_write_instr i =
    match i with
    | DoubleInstr (p, _, _, _, _, _) when is_push p -> Some DOUBLE_WRITE
  ...
  let is_mem_read_instr instr =
    match instr with
    | SingleInstr (op, _, _, _, _) when is_ret instr ->
        (* ret *)
        Some SINGLE_READ
    | DoubleInstr (_, _, _, _, _, _) when is_arm_ret instr -> Some DOUBLE_READ
    | DoubleInstr (op, _, _, _, _, _) when is_stack_op op ->
        (* push & pop *)
        Some DOUBLE_READ
    | TripleInstr (op, _, e2, _, _, _, _) when is_mem_exp e2 -> begin
        match op with
        | Intel_OP io -> begin
            match io with
            | Intel_CommonOP (Intel_Assign io)
              when io = LEA || io = LEAL || io = LEAQ ->
                None
  ...
  let is_addr_or_label instr aols =
    ...
    let help aol =
      if String.get aol 0 = '0' && String.get aol 1 = 'x' then
        (* process address *)
        let addr = int_of_string aol in
        if iloc.loc_addr = addr then true else false
      else
        (* process label *)
        let contains s1 s2 =
        ...
        if contains iloc.loc_label aol then true else false
    in
  ...
  let is_jmp_instr i =
    match i with
    | SingleInstr (p, _, _, _, _) when is_ret i -> Some RET_TYPE
    | DoubleInstr (_, _, _, _, _, _) when is_arm_ret i -> Some RET_TYPE
    | DoubleInstr (_, e, _, _, _, _) when is_indirect e -> Some INDIRECT
    | DoubleInstr (p, _, _, _, _, _) when is_call p -> Some DIRECT_CALL
...
module Instr_visitor = struct
  let map_instr judge visitor il =
    let aux i = match judge i with Some t -> visitor i t | None -> i in
  ...
  let map_instr' judge visitor il labels =
    let rec aux il visited acc =
      ...
      | h :: t -> (
          ...
          if List.mem h_addr visited then aux t visited (h :: acc)
          else
            match judge h labels with
            | true ->
                let i' = visitor h in
                aux t (h_addr :: visited) (i' :: acc)
            | false -> aux t (h_addr :: visited) (h :: acc))
    in
    aux il [] []
end
...
module Instr_template = struct
  let gen_logging_instrs i iloc =
    ...
    let i3 =
      DoubleInstr
        ( Intel_OP (Intel_ControlOP (Intel_Loop LOOP)),
          Label ads,
          iloc',
          None,
          None,
    ...
    let i5 =
      TripleInstr
        ( Intel_OP (Intel_CommonOP (Intel_Assign MOVL)),
          Ptr (JmpTable_PLUS_S ("buf", Intel_Reg (Intel_CommonReg ECX), 4)),
          Const (Normal addr'),
          { iloc' with loc_label = ads ^ ":" },
          None,
          None,
          Hashtbl.create 0 )
    in
    ...
    set_update_fold i7 iloc [] |> set_update_fold i6 iloc
    |> set_update_fold i5 iloc
    (* |> set_update_fold i4 iloc *)
    |> set_update_fold i3 iloc
    |> set_update_fold i2 iloc |> set_update_fold i1 iloc |> List.flatten
end
...
module BB_utils = struct
  ...
  let bb_map bbl instrs =
    ...
    let rec aux bl il acc =
      match (bl, il) with
      | bh :: bt, ih :: it ->
          let bloc = bh.bblock_begin_loc and eloc = bh.bblock_end_loc in
          ...
          if ia = ea then begin
            ...
            ih :: acc |> List.rev |> Hashtbl.replace m n;
            aux bt it []
          end
  ...
  let bbl_sort bbl =
    List.sort
      (fun b1 b2 -> b1.bblock_begin_loc.loc_addr - b2.bblock_end_loc.loc_addr)
      bbl
...
module Func_utils = struct
  ...
  let func_sort fl =
    fl
    |> List.filter (fun f -> f.is_lib = false)
    |> List.sort (fun b1 b2 -> b1.func_begin_addr - b2.func_begin_addr)
  ...
  let caller_collect fn instrs =
    let is_call op =
      ...
      | Intel_OP io -> begin
          match io with
          | Intel_ControlOP ico -> begin
              match ico with CALL -> true | _ -> false
            end
          | _ -> false
        end
      | Arm_OP (ao, _, _) -> begin
    ...
    let help acc i =
      ...
      | DoubleInstr (p, e, _, _, _, _) when is_call p ->
          let open Pp_print in
          let es = p_exp e in
          if String.exists es fn then i :: acc else acc
      | _ -> acc
    in
    ...
    let aux f (pred_cfg, succ_cfg, _) =
      Hashtbl.iter
        (fun i_from i_tos ->
          ...
          | Some i_from, i_tos -> begin
              print_endline (pp_print_instr' i_from ^ " p->");
              List.iter
                (fun i_to ->
                  match i_to with
                  | Some i_to -> print_endline (">>> " ^ pp_print_instr' i_to)
  ...
  let func2cfg (il : instr list) (funcs : func list) : (string, cfgi) Hashtbl.t
      =
    let is_ct op =
      match op with
      | Intel_OP io -> (
          match io with
          | Intel_ControlOP ico -> (
              match ico with Intel_Jump _ -> true | _ -> false)
          | _ -> false)
      | Arm_OP (ao, _, _) -> (
    ...
    in
    let fb2fn funcs d =
      (* find the function name with the function begin address *)
    ...
    let func2il il =
      ...
      let rec slice_il (fl : func list) (il : instr list) :
        ...
        | hf :: tf, hi :: ti -> begin
            ...
            (* end debug *)
            if i_addr >= f_ba && i_addr < f_ea then begin
              ...
              let _ =
                match get_ct_des hi with
                | Some d ->
                    if d >= f_ea then begin
                      match fb2fn funcs d with
                      | Some fn -> Queue.push (hf.func_name, fn) worklist
                      | None -> ()
                    end
                | None -> ()
              in
      ...
      (*       List.iter *)
      (*         (fun i -> Printf.printf "\t%s\n%!" (Pp_print.pp_print_instr' i)) *)
      (*         v) *)
      (*     func2il' *)
      (* in *)
      (* Instructions are reserve-ordered and will be adjusted in [create_cfg] *)
      (* end debug *)
      ...
      func2il'
    in
    let add_edge curr_cfg (i_from : instr option) (i_to : instr option) :
      ...
      else Hashtbl.replace curr_cfg i_from [ i_to ];
      curr_cfg
    in
    ...
    let rec create_cfg f (f_il : instr list) pred_cfg succ_cfg =
      ...
      | i :: i' :: il' ->
          ...
          let _ =
            ...
            | Intel_OP (Intel_ControlOP RET)
            | Intel_OP (Intel_ControlOP RETN)
            | Intel_OP (Intel_ControlOP RETQ)
            | Intel_OP (Intel_ControlOP LEAVE)
          ...
          let pred_cfg =
            ...
            | Some d -> begin
                let ils_full : instr list =
                  Hashtbl.find func2il_table f.func_name
                in
                let des_is : instr list =
                  List.filter (fun l -> (get_loc l).loc_addr = d) ils_full
                in
                match des_is with
                | [] -> pred_cfg
    ...
    List.iter
      (fun f ->
        ...
        | Some f_il ->
            create_cfg f (List.rev f_il) (Hashtbl.create 40) (Hashtbl.create 40)
        | None -> ())
      funcs;
    func2cfg_table
...
module Dataset_utils = struct
  let insert_data label value size first_time label_pos =
    ...
    Sys.command
      ("python3 data_instrumentation.py " ^ label ^ " " ^ value ^ " " ^ size
     ^ " " ^ first_time ^ " " ^ label_pos)
end
...
module Cfg_utils = struct
        ...
        if ic = true then acc1
          ...
          List.fold_left
            (fun acc s ->
              ...
              | _, Some d when List.mem d dl' -> begin
  ...
  let map_jmp (visitor : instr -> jmp_type -> instr) instrs : instr list =
    let aux i =
      match i with
      | SingleInstr (p, _, _, _, _) when is_ret i -> visitor i RET_TYPE
      | DoubleInstr (_, _, _, _, _, _) when is_arm_ret i -> visitor i RET_TYPE
      | DoubleInstr (_, e, _, _, _, _) when is_indirect e -> visitor i INDIRECT
      | DoubleInstr (p, _, _, _, _, _) when is_call p -> visitor i DIRECT_CALL
      | DoubleInstr (p, e, _, _, _, _) when is_jmp i && is_func e = true ->
  ...
  let map_mem_write (visitor : instr -> mem_write_type -> instr) instrs :
      instr list =
    let aux i =
      match i with
      | DoubleInstr (p, _, _, _, _, _) when is_push p -> visitor i DOUBLE_WRITE
      (* memory assignment operation *)
      | TripleInstr (op, e1, _, _, _, _, _) when is_assign op && is_mem_exp e1
        ->
...
module Cg_utils = struct
  let show_call_graph cg =
    let help key value =
      ...
      List.iter (fun f -> print_endline ("    " ^ f.func_name)) value
    in
  ...
  let show_cfi_sepcified_graph cfi_cg =
    let help key value =
      print_endline key;
      List.iter (fun l -> print_endline ("    " ^ dec_hex l)) value
    in
    Hashtbl.iter help cfi_cg


  (* visitor function for CFI transformation *)
  let map_cfi_des cfi_cg fn visit = Hashtbl.find cfi_cg fn |> List.map visit

  (* visitor function for CFI transformation *)
  let map_cfi_des_cond cfi_cg fn visit judge =
    Hashtbl.find cfi_cg fn |> List.filter judge |> List.map visit
end
...
module Time_Record = struct
  ...
  let get_utime () = Unix.gettimeofday ()
