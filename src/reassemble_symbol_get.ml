    ...
    method data_output =
      let p = object(sp)
        ...
        method insert_head =
          ...
          rodata_cst32_list <- (".section rodata.cst32\n", "")::rodata_cst32_list;
          let module EU = ELF_utils in
          if EU.elf_static () then
            begin
    ...
    *)
    ...
    method build_plt_symbol c =
      ...
      try
        ...
        | Normal _ -> ("$"^n)
      with _ -> failwith (Printf.sprintf "exception in build plt symbol: %d\n" c')
      ...
      let is_test op = match op with
        | Intel_OP io ->
          begin
            ...
            | Intel_CommonOP (Intel_Compare TESTB) -> true
            | _ -> false
          end
        | Arm_OP _ -> false in
      match i with
