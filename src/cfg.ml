...
class cfg =
  ...
  object(self)
    ...
    val mutable bl_sort = Array.make 1 {bn =""; baddr = 0; eaddr = 0;}
	...
	*)
    ...
    method update_bl =
      ...
      let rec assign fl bl acc =
        ...
        | ([], hb::tb) ->
		   (
        (* If we don't have symbols, extracted funcs may not be correct.
         * Therefore, bb may not have matching func *)
        assign [] tb (hb::acc)
		   ) in
      assign funcs' bls' []
     ...
     *)
    method recover_cfg =
      let is_jmp = Opcode_utils.is_jmp in
      let is_cond_jmp = Opcode_utils.is_cond_jmp in
      let is_call = Opcode_utils.is_call in
      let is_ret = Opcode_utils.is_ret in
      let is_arm_ret = Opcode_utils.is_arm_ret in
      let is_indirect = Opcode_utils.is_indirect in
