...
type stack_type = Op of op | Exp of exp | Loc of loc
...
class arm_parse =
  ...
  and pcreg_symb = function
    | "r15" -> R15 | "pc" -> PC | _ -> raise ParseError
  in
    ...
    with _ -> (
      ...
      with _ -> (
        ...
        with _ -> (
          ...
          with _ -> (
            try Arm_PCReg (pcreg_symb r') with _ -> raise ParseError))))
  ...
  let binptr_p_symb s =
    if not (String.contains s ',') then raise ParseError
      ...
      let bracket_removed_s =
        ...
        (* LDR r0, [sp,#4]! *)
        if s.[String.length s - 1] = '!' then raise ParseError
      ...
      let offset =
        String.sub offset_with_hash 1 (String.length offset_with_hash - 1)
      in
      (* Handle minus offset in binptr_m_symb function *)
      if offset.[0] = '-' then raise ParseError
      else (reg_symb reg, int_of_string offset)
  in
  ...
  let binptr_p_r_symb s =
    if not (String.contains s ',') then raise ParseError
      ...
      (reg_symb first_reg, reg_symb second_reg)
  in
  ...
  let binptr_p_wb_symb s =
    if not (String.contains s ',') then raise ParseError
      ...
      let bracket_removed_s =
        ...
        (* LDR r0, [sp,#4]! *)
        if s.[String.length s - 1] <> '!' then raise ParseError
        else String.sub s 1 (String.length s - 3)
      in
      let split = Str.split (Str.regexp_string ",") in
      let items = split bracket_removed_s in
      if List.length items <> 2 then raise ParseError;
  ...
  let binptr_m_symb s =
    if not (String.contains s ',') then raise ParseError
      ...
      if offset.[0] <> '-' then raise ParseError
        ...
        (reg_symb reg, int_of_string unsigned_offset)
  in
  ...
  let threeptr_s_symb s =
    let bracket_removed_s =
      if s.[String.length s - 1] = ']' then String.sub s 1 (String.length s - 2)
  ...
  and control_symb = function
    ...
    | "retaasppc" -> RETAASPPC
    | "retabsppc" -> RETABSPPC
    | "retaasppcr" -> RETAASPPCR
    | "retabsppcr" -> RETABSPPCR
    | _ -> raise ParseError
  in

  let controlop_symb = function
    | s -> ( try control_symb s with _ -> raise ParseError)
  ...
  let errorop_symb = function
    | "(bad)" -> BAD | _ -> failwith "unsupported type in errorop_symb"
  in

  let commonop_symb (s : string) (is_special : bool) =
    if is_special then raise ParseError
    else
        ...
        with _ -> (
          ...
          with _ -> (
            ...
            with _ -> (
              ...
              with _ -> (
                try Arm_Other (otherop_symb s) with _ -> raise ParseError)))))
  in
      ...
      with _ -> (
        ...
        with _ -> (
          ...
          with _ -> (
            ...
            with _ -> (
              try Arm_SystemOP (systemop_symb s) with _ -> raise ParseError)))))
  in
  ...
  object (self)
    ...
    method jumpdes_symb_bak s =
      ...
      let s' = String.trim s1 and split = Str.split (Str.regexp_string "+") in
      let items = split s' in
      let offset = List.nth items 1 and func = List.nth items 0 in
    ...
    method jmptable_m_symb (s : string) =
      if String.exists s "(," then
        ...
        if addr.[0] <> '-' then raise ParseError
          ...
          let r = List.nth tokens 1 and off = List.nth tokens 2 in
          let addr' = String.sub addr 1 (String.length addr - 2) in
          let off' = String.sub off 0 (String.length off - 1) in
          (int_of_string addr', reg_symb r, int_of_string off')
      else raise ParseError
    ...
    method symbol_symb s =
      ...
      else if !call_des = true then CallDes (self#calldes_symb s')
        (* else if String.contains s '+' then *)
      else self#jumpdes_symb s'
  ...
  method push_stack lex (is_special : bool) =
    ...
    method reduce_stack stack pre (tag : tag option) =
      ...
      | [ Loc l; Exp exp1; Op p ] ->
          DoubleInstr (p, exp1, l, pre, tag, Hashtbl.create 0)
      | [ Loc l; Exp exp1; Exp exp2; Op p ] ->
          TripleInstr (p, exp1, exp2, l, pre, tag, Hashtbl.create 0)
      | [ Loc l; Exp exp1; Exp exp2; Exp exp3; Op p ] ->
          FourInstr (p, exp1, exp2, exp3, l, pre, tag, Hashtbl.create 0)
      | [ Loc l; Exp exp1; Exp exp2; Exp exp3; Exp exp4; Op p ] ->
          FifInstr (p, exp1, exp2, exp3, exp4, l, pre, tag, Hashtbl.create 0)
      | _ -> raise ParseError
    ...
    method set_funclist l = func_list <- l
    method get_funclist = func_list
    method set_seclist l = sec_list <- l
    ...
    method get_sec addr =
      let rec help l addr =
        ...
        | h :: t ->
            ...
            if addr >= b && addr < e then h else help t addr
        | [] ->
            print_string "error in get_sec";
            raise ParseError
      in
      help sec_list (int_of_string addr)

    method init_process = call_des := false
