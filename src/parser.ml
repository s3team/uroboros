...
class parse =
  ...
  and specialreg_symb (s : string) : intel_specialreg = match s with
    | "r8" -> R8 | "r9" -> R9 | "r10" -> R10
  ...
  let reg_symb r =
    ...
    try Intel_CommonReg (commonreg_symb r')
    with _ ->
      try Intel_SpecialReg (specialreg_symb r')
      with _ ->
        try Intel_StackReg (stackreg_symb r')
        with _ ->
          try Intel_PCReg (pcreg_symb r')
          with _ ->
            try Intel_OtherReg (otherreg_symb r')
...
and remove_parenthesis s' =
  ...
  and seg_symb (s : string) : intel_seg = match s with
    | "fs" -> FS | "gs" -> GS | "cs" -> CS
    | "ss" -> SS | "ds" -> DS | "es" -> ES
    | _ -> raise ParseError  in
  ...
  and arithm_symb (s : string) : intel_arithmop = match s with
    | "adc" -> ADC | "add" -> ADD | "xadd" -> XADD | "addl" -> ADDL | "addq" ->ADDQ | "adcq" -> ADCQ
    | "addsd" -> ADDSD | "addss" -> ADDSS | "subss" -> SUBSS
    | "sub" -> SUB | "mul" -> MUL | "imul" -> IMUL | "mulsd" -> MULSD | "psubq" -> PSUBQ
    ...
    | _ -> raise ParseError
  and logicop_symb (s : string) : intel_logicop = match s with
    ...
    | "notq" -> NOTQ | "andnpd" -> ANDNPD | "orpd" -> ORPD | "pand" -> PAND | "shlx" -> SHLX
    | "por" -> POR | "pandn" -> PANDN | "vpxor" -> VPXOR | "vpxord" -> VPXORD | "vpxorq" -> VPXORQ | "vporq" -> VPORQ
    | "vpand" -> VPAND | "vpternlogd" -> VPTERNLOGD | "vptestmb" -> VPTESTMB | "vptestnmb" -> VPTESTNMB | "kxnorq" -> KXNORQ
    | "kortestd" -> KORTESTD | "kord" -> KORD | "andn" -> ANDN | "kortestq" -> KORTESTQ | "blsr" -> BLSR | "vptestnmd" -> VPTESTNMD
    | _ -> raise ParseError
  and rolop_symb = function
    | "rol" -> ROL | "shl" -> SHL | "shr" -> SHR | "shrl" -> SHRL
    | "shld" -> SHLD | "shrd" -> SHRD | "roll" -> ROLL | "shrb" -> SHRB
    | "sal" -> SAL | "sar" -> SAR | "shll" -> SHLL | "shlb" -> SHLB | "shrx" -> SHRX
    | "sarl" -> SARL | "ror" -> ROR | "rorl" -> RORL
    ...
    | "sarq" -> SARQ | "sarx" -> SARX
    | _ -> raise ParseError
  and assignop_symb (s : string) : intel_assignop = match s with
    | "mov" -> MOV | "movaps" -> MOVAPS | "vmovaps" -> VMOVAPS | "xchg" -> XCHG | "lea" -> LEA | "leal" -> LEAL | "leaq" -> LEAQ
    | "movsx" -> MOVSX | "movapd" -> MOVAPD | "movslq" -> MOVSLQ | "movq" -> MOVQ | "movabs" -> MOVABS | "movbe" -> MOVBE
    | "movsd" -> MOVSD | "movsw" -> MOVSW | "movsb" -> MOVSB | "movss" -> MOVSS | "fstpl" -> FSTPL | "lddqu" -> LDDQU
    | "movzx" -> MOVZX | "fld" -> FLD | "fstp" -> FSTP | "movl" -> MOVL | "fldl" -> FLDL
    | "movntdq" -> MOVNTDQ | "vmovntdq" -> VMOVNTDQ
    ...
    | "fcmovb" -> FCMOVB | "fistp" -> FISTP | "fcmovnb" -> FCMOVNB | "movntps" -> MOVNTPS
    ...
    | "punpcklbw" -> PUNPCKLBW | "punpckhdq" -> PUNPCKHDQ
    ...
    | "bsf" -> BSF
    | "xgetbv" -> XGETBV | "pmovmskb" -> PMOVMSKB | "vpmovmskb" -> VPMOVMSKB | "pminub" -> PMINUB | "pminud" -> PMINUD | "vpminub" -> VPMINUB | "vpminud" -> VPMINUD | "palignr" -> PALIGNR | "vpalignr" -> VPALIGNR
    | "tzcnt" -> TZCNT | "vzeroupper" -> VZEROUPPER | "pcmpistri" -> PCMPISTRI | "pmaxub" -> PMAXUB | "vpmaxub" -> VPMAXUB
    | "vpbroadcastb" -> VPBROADCASTB | "vpbroadcastd" -> VPBROADCASTD | "vpbroadcastq" -> VPBROADCASTQ | "vbroadcastss" -> VBROADCASTSS
    | "vpor" -> VPOR | "vpand" -> VPAND | "vpandn" -> VPANDN | "vptest" -> VPTEST
    | "vpcmpeqb" -> VPCMPEQB | "vpcmpeqd" -> VPCMPEQD | "vpcmpeqq" -> VPCMPEQQ | "vpcmpgtb" -> VPCMPGTB | "vpcmpistri" -> VPCMPISTRI
    | "vpshufb" -> VPSHUFB | "vpslldq" -> VPSLLDQ | "vpsrldq" -> VPSRLDQ | "vpsubb" -> VPSUBB
    | "bts" -> BTS | "std" -> STD | "cld" -> CLD | "rdsspd" -> RDSSPD | "rdsspq" -> RDSSPQ
    | "movmskps" -> MOVMSKPS | "movmskpd" -> MOVMSKPD | "bndmov" -> BNDMOV
    | "fxsave" -> FXSAVE | "fxrstor" -> FXRSTOR | "xrstor" -> XRSTOR | "fxam" -> FXAM | "xsave" -> XSAVE | "xsavec" -> XSAVEC
    | "incsspq" -> INCSSPQ | "btr" -> BTR | "fldenv" -> FLDENV | "fnstenv" -> FNSTENV | "stmxcsr" -> STMXCSR | "incsspd" -> INCSSPD | "ldmxcsr" -> LDMXCSR
    ...
    | _ -> raise ParseError
  and compareop_symb (s : string) : intel_compareop = match s with
    | "cmp" -> CMP | "cmpq" -> CMPQ | "test" -> TEST | "cmpl" -> CMPL
    | "cmpb" -> CMPB | "cmpw" -> CMPW | "testb" -> TESTB | "ptest" -> PTEST
    | "testl" -> TESTL | "cmpsb" -> CMPSB | "bt" -> BT | "btc" -> BTC | "btcq" -> BTCQ
    | "testw" -> TESTW | "cmpnless" -> CMPNLESS | "cmpltss" -> CMPLTSS
    | "cmpltsd" -> CMPLTSD | "cmpnlesd" -> CMPNLESD | "cmpxchg" -> CMPXCHG
  ...
  and setop_symb = function
    | "seta" ->  SETA | "setae" -> SETAE | "setb" -> SETB | "setc" -> SETC | "seto" -> SETO
    ...
    | "sets" -> SETS
    | _ -> raise ParseError
  and otherop_symb (s : string) : intel_otherop = match s with
    | "nop" -> NOP | "hlt" -> HLT | "nopw" -> NOPW | "nopl" -> NOPL | "ud2" -> UD2 | "endbr32" -> ENDBR32 | "endbr64" -> ENDBR64 | "cpuid" -> CPUID | "pause" -> PAUSE
    | "xend" -> XEND | "vzeroall" -> VZEROALL
    | _ -> raise ParseError

  and stackop_symb (s : string) : intel_stackop = match s with
    | "push" -> PUSH | "pop" -> POP
  ...
  let controlop_symb = function s ->
    try Intel_Jump (jumpop_symb s) with _ ->
      try Intel_Loop (loopop_symb s) with _ ->
        try Intel_Flag (flagop_symb s) with _ ->
          try (control_symb s) with  _ ->
            raise ParseError

  and systemp_symb = function
    | "int" -> INT | "in" -> IN | "syscall" -> SYSCALL | "xabort" -> XABORT
    | "prefetchnta" -> PREFETCHNTA | "prefetcht0" -> PREFETCHT0 | "prefetcht1" -> PREFETCHT1
    | "sfence" -> SFENCE | "lfence" -> LFENCE | "fwait" -> FWAIT
    | "out" -> OUT | _ -> raise ParseError in

  let errorop_symb (s : string) : intel_errorop = match s with
    | "(bad)" -> BAD
    | _ -> failwith "unsupported type in errorop_symb" in

  let commonop_symb = function s ->
    try Intel_Arithm (arithm_symb s)
    with _ ->
      try Intel_Logic (logicop_symb s)
      with _ ->
        try Intel_Rol (rolop_symb s)
        with _ ->
          try Intel_Assign (assignop_symb s)
          with _ ->
            try Intel_Compare (compareop_symb s)
            with _ ->
              try Intel_Set (setop_symb s)
              with _ ->
                try Intel_Other (otherop_symb s)
...
| "stos" -> STOS
  ...
  let op_symb = function s ->
    try Intel_ErrorOP (errorop_symb s)
    with _ ->
      try Intel_CommonOP (commonop_symb s)
      with _ ->
        try Assist (assist_symb s)
        with _ ->
          try Intel_StackOP (stackop_symb s)
          with _ ->
            try Intel_ControlOP (controlop_symb s)
            with _ ->
              try Intel_SystemOP (systemp_symb s)
              with _ ->
                (* identify all instructions that need parsing support *)
                (* (print_string "parse error for instr: ";
                print_endline s;
                Intel_CommonOP (Intel_Other NOP)) in *)
                raise ParseError in
...
let eiz_symb = function s ->
  ...
  object (self)
    ...
    method ptraddr_symb s =
      try UnOP (Intel_Reg (unptr_symb s))
      with _ ->
        try
          let (r, i) = (binptr_m_symb s) in
          BinOP_MINUS (Intel_Reg r, i)
        with _ ->
          try
            let (r, i) = (binptr_p_symb s) in
            BinOP_PLUS (Intel_Reg r, i)
          with _ ->
            try
              let (r1, r2, i) = (threeptr_symb s) in
              ThreeOP (Intel_Reg r1, Intel_Reg r2, i)
            with _ ->
              try
                let (r1, r2, i1, i2) = (fourptr_m_symb s) in
                FourOP_MINUS (Intel_Reg r1, Intel_Reg r2, i1, i2)
              with _ ->
                try
                  let (r1, r2, i1, i2) = (fourptr_p_symb s) in
                  FourOP_PLUS (Intel_Reg r1, Intel_Reg r2, i1, i2)
                with _ ->
                  try
                    let (i1, r, i2) = (self#jmptable_m_symb s) in
                    JmpTable_MINUS (i1, Intel_Reg r, i2)
                  with _ ->
                    try
                      let (s, r, i) = (self#jmptable_p_symb s) in
                      JmpTable_PLUS (s, Intel_Reg r, i)
                    with _ ->
                      try
                        let (seg, r) = (segref_symb s) in
                        SegRef (seg, Intel_Reg r)
                      with _ -> raise ParseError
    ...
    method get_func name lib =
      let rec help l name =
        ...
        | [] ->
          let func' : func =
            {func_name=name; func_begin_addr=0; func_end_addr = 0; is_lib=lib;}
          in
    ...
    method parse_instr instr loc (arch : string) =
      self#init_process;
      let pre = prefix_identify instr
      and instr' = prefix_sub instr in
      let lexem_list = Array.to_list (lexer instr' loc arch) in
